...
public class ConsultaPersonas implements WindowListener, ActionListener {
	...
	public ConsultaPersonas()
	{
		...
		txaListado.setText(bd.consultarPersonas());
		//Desconectar
		
		String usuario;
		usuario= Login.txtUsuario.getText();
		bd.registroEscritura("["+usuario+"]"+"[SELECT * from personas]");
		System.out.println("["+usuario+"]"+"[SELECT * from personas]");
		...
		try
		{
			...
			while(resultSet.next())
			{
				...
				document.add(new Paragraph((resultSet.getInt("idPersona")+
						...
						"-"+resultSet.getInt("edadPersona") +
						"-"+ resultSet.getString("domicilioPersona") +
						"-" + resultSet.getString("emailPersona")+ "-"+
						resultSet.getInt("telefonoPersona")+"\n")));
			}
