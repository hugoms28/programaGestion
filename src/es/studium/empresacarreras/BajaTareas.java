...
public class BajaTareas implements WindowListener, ActionListener {
	Frame ventana = new Frame("Baja de Tarea");
	Label lblCabecera = new Label("Elegir la Tarea a Borrar");
	...
	public void actionPerformed(ActionEvent evento) 
	{
		...
		lblConfirmacion.setText("¿Seguro que desea borrar la tarea" + choTareas.getSelectedItem()+"?");
		...
		else if (evento.getSource().equals(btnSi))
		{
			...
			bd.conectar();
			//Hacer el DELETE
			String[] array = choTareas.getSelectedItem().split("-");
			int resultado = bd.borrarTarea(Integer.parseInt(array[0]));
			if(resultado == 0)
			{
				lblMensaje.setText("Borrado con éxito");
				String usuario;
				usuario= Login.txtUsuario.getText();
				bd.registroEscritura("["+usuario+"]"+"[DELETE FROM tareas]");
				System.out.println("["+usuario+"]"+"[DELETE FROM tareas]");
			}
			else
			{
