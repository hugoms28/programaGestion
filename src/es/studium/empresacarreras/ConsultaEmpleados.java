...
public class ConsultaEmpleados implements WindowListener, ActionListener {
	
	Frame ventana = new Frame("Listado de Empleados");
	TextArea txaListado = new TextArea(6,34);
	Button btnPdf = new Button ("Exportar a PDF");
	BaseDatos bd = new BaseDatos();
	
	public static final String DEST = "ConsultaEmpleados.pdf";
	PdfFont font;
	PdfWriter writer;
	...
	public ConsultaEmpleados()
	{
		...
		txaListado.setText(bd.consultarEmpleados());
		//Desconectar
		bd.desconectar();
		String usuario;
		usuario= Login.txtUsuario.getText();
		bd.registroEscritura("["+usuario+"]"+"[SELECT * from empleados]");
		System.out.println("["+usuario+"]"+"[SELECT * from empleados]");
	...
	{if(evento.getSource().equals(btnPdf))
	{
		...
		try
		{
			...
			sentencia = "SELECT * FROM empleados join personas on idPersona = idPersonaFK;";
			resultSet = statement.executeQuery(sentencia);
			// Mostramos en choice los clientes de la empresa
			while(resultSet.next())
			{
				//Guardamos en documento PDF
				document.add(new Paragraph((resultSet.getInt("idEmpleado")+
						"-"+resultSet.getString("salarioEmpleado") +
						"-" + resultSet.getString("apellidosPersona")+
						"-"+resultSet.getInt("idPersonaFK") +"\n")));
			}
