...
public class ConsultaProveedores implements WindowListener, ActionListener {
	...
	Frame ventana = new Frame("Listado de Proveedores");
	TextArea txaListado = new TextArea(6,34);
	Button btnPdf = new Button ("Exportar a PDF");
	BaseDatos bd = new BaseDatos();
	
	public static final String DEST = "ConsultaProveedores.pdf";
	PdfFont font;
	PdfWriter writer;
	...
	public ConsultaProveedores()
	{
		...
		txaListado.setText(bd.consultarProveedores());
		//Desconectar
		bd.desconectar();
		String usuario;
		usuario= Login.txtUsuario.getText();
		bd.registroEscritura("["+usuario+"]"+"[SELECT * from proveedores]");
		System.out.println("["+usuario+"]"+"[SELECT * from proveedores]");
	...
	{if(evento.getSource().equals(btnPdf))
	{
		...
		try
		{
			...
			sentencia = "SELECT * FROM proveedores join personas on idPersona = idPersonaFK;";
			resultSet = statement.executeQuery(sentencia);
			// Mostramos en choice los clientes de la empresa
			while(resultSet.next())
			{
				//Guardamos en documento PDF
				document.add(new Paragraph((resultSet.getInt("idProveedor")+
